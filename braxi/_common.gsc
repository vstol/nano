...
canSpawn()
{
	if( level.freeRun || level.training || self.pers["lifes"] )
		return true;
...
made_by_Sajjad_new_ending_hud( align, fade_in_time, x_off, y_off ,who)
{
...
madebySajjad( start_offset, movetime, mult, text,who )
{
	start_offset *= mult;
	hud = made_by_Sajjad_new_ending_hud( "center", 0.1, start_offset, 0,who );
	hud setText( text );
	hud moveOverTime( movetime );
	hud.x = 0;
	wait( movetime );
