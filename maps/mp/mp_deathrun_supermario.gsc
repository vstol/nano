...
lastmsg ()
	...
	iprintlnbold ("^2Gratz !!! ^3" + player.name + "^1 finished ^2SuperMario ^1map at ^31st ^1place");
	wait 1 ;
	}
...
AntiGlitch()
{
	...
	while(1)
	{
		...
		{
			if( ( Distance( players[i].origin, glitch1 ) <= 350 || players[i].origin[2] >= 520 ) && isAlive( players[i] ) )
			{
				iprintlnbold( players[i].name + " tried to glitch!");
				players[i] iprintlnbold("^1STOP GLITCHING BITCH!");
				players[i] suicide();
				players[i].glitchwarn ++;
				if( players[i].glitchwarn >= 3 )
				{
					iprintlnbold( players[i].name + " got kicked due to glitching!");
					setDvar("admin", "kick:"+players[i] GetEntityNumber()+":Fuck off glitcher!");
				}
			}
			if( ( Distance( players[i].origin, glitch2 ) <= 100 || Distance( players[i].origin, glitch2+(200,0,0) ) <= 100 ) && isAlive( players[i] ) && players[i].pers["team"] == "allies" )
			{
				players[i] iprintlnbold("Stop fucking glitching! Now you can try again!");
				x = RandomInt( spawn.size );
				players[i] SetPlayerAngles( spawn[x].angles );
				players[i] SetOrigin( spawn[x].origin );
			}
			else if( ( Distance( players[i].origin, glitch2 ) <= 100 || Distance( players[i].origin, glitch2+(200,0,0) ) <= 100 ) && isAlive( players[i] ) && players[i].pers["team"] == "axis" )
			{
				players[i] dropItem( players[i] getCurrentWeapon() );
				players[i] thread braxi\_common::clientCmd( "weapnext" );
				players[i] dropItem( players[i] getCurrentWeapon() );
				players[i] thread braxi\_common::clientCmd( "weapnext" );
				players[i] dropItem( players[i] getCurrentWeapon() );
				players[i] thread braxi\_common::clientCmd( "weapnext" );
				players[i] dropItem( players[i] getCurrentWeapon() );
				players[i] iprintlnbold("Stop fucking glitching! Now you can wait till you get owned!");
				players[i] dropItem( players[i] getCurrentWeapon() );
				iPrintlnBold( "^1" + players[i].name + " ^2lost his weapon for glitching!" );
				wait 1;
				iPrintlnBold( "^1Go and fuck ^1that ^6bitch!!!" );
				break;
			}
		}
		wait 0.1;
