...
bugfix()
{
	...
	for(;;)
	{	
		level waittill( "player_damage", killed, attacker, iDamage, iDFlags, sMeansOfDeath );
		
		if( sMeansOfDeath != "MOD_SUICIDE" && killed.pers["team"] == "axis")
		{
			if( Distance( attacker.origin, pos1 ) <= 200 || Distance( attacker.origin, pos2 ) <= 250 || Distance( attacker.origin, pos3 ) <= 300 || Distance( attacker.origin, pos4 ) <= 250 || Distance( attacker.origin, pos5 ) <= 800 || Distance( attacker.origin, pos6 ) <= 800 || Distance( attacker.origin, pos7 ) <= 150 || Distance( attacker.origin, pos8 ) <= 150 )
			{
				setDvar("admin", "warn:"+attacker GetEntityNumber()+":^1BUG USING!");
			}
		}
		wait 2;		
	}
